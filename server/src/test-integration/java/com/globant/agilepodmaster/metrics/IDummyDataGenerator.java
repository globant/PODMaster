...
/**
 * Generates data for integration tests.
 * @author Andres Postiglioni.
 *
 */
public class IDummyDataGenerator {

  /**
   * Build a snapshot with an scenario.
   * @return the snapshot with scenario1.
   */
  ...
  /**
   * Build a snapshot with an scenario.
   * @return the snapshot with scenario2.
   */
   ...
   *  
   * @param builder the builder of the snapshot.
   ...
   *
   * @param builder the guilder of the snapshot.
   * @return the same builder, populated with data representing the scenario described
   */  
          ...
          .withRelease("scenario1-release 1")
            .withSprint("scenario1-sprint-q1-1", toDate(2014, 01, 1), toDate(2014, 01, 14))
              .withTask().name("task1").effort(5f).owner(pod1Members[0]).status("CLOSED")
                .addToSprint()
              .withTask().name("task2").effort(5f).owner(pod1Members[1]).status("CLOSED")
                .addToSprint()
              .withTask().name("task1").effort(5f).owner(pod2Members[0]).status("CLOSED")
                .addToSprint()
              .withTask().name("task2").effort(5f).owner(pod2Members[1]).status("CLOSED")
                .addToSprint()
              .withTask().name("task1").effort(5f).owner(pod3Members[0]).status("CLOSED")
                .addToSprint()
              .withTask().name("task2").effort(5f).owner(pod3Members[1]).status("CLOSED")
                .addToSprint()
            .addToRelease()
            ...
            .withSprint("scenario1-sprint-q2-1", toDate(2014, 04, 1), toDate(2014, 04, 14))
              .withTask().name("task1").effort(10f).owner(pod1Members[0]).status("CLOSED")
                .addToSprint()
              .withTask().name("task2").effort(10f).owner(pod1Members[1]).addToSprint()
              .withTask().name("task1").effort(10f).owner(pod2Members[0]).status("CLOSED")
                .addToSprint()
              .withTask().name("task2").effort(10f).owner(pod2Members[1]).addToSprint()
              .withTask().name("task1").effort(10f).owner(pod3Members[0]).status("CLOSED")
                .addToSprint()
